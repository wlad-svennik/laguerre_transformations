...
class Display(Thread):
    """This class displays an animated sequence of images in a new window.

    Example:

        > Display(imgs).start()
        displays the images stored in imgs in a new window."""

    def __init__(self, images):
        """Takes in a sequence of Pillow images, and produces a thread
        that displays that sequence in an animation."""
        self.images = images
    ...
    def run(self):
        ...
        filemenu.add_command(label="Save As...",
                             command=lambda: save_animation(self.images))
        menubar.add_cascade(label="File", menu=filemenu)
        lbl = ImageLabel(root)
        lbl.pack()
        lbl.load(self.images)
